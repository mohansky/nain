...
import type { ChildSubmissionData, ChildWithRelation } from "@/types";
...
export default function ChildrenDashboardPage() {
  ...
  const { children, isLoading, error, addChild, updateChild, deleteChild } =
    useChildren();

  const [viewMode, setViewMode] = useState<ViewMode>("list");
  const [editingChild, setEditingChild] = useState<ChildWithRelation | null>(
    null
  );
  ...
  const handleAddChild = async (childData: ChildSubmissionData) => {
  ...
  const handleUpdateChild = async (childData: ChildSubmissionData) => {
